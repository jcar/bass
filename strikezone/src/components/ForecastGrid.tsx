...
export default function ForecastGrid({ forecast, selectedDay, onDaySelect, loading, compact }: ForecastGridProps) {
  ...
  return (
    <div className={compact ? 'px-4 py-2' : 'p-3'}>
      {/* Forecast grid */}
        ...
        {forecast.map((day, i) => {
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 rounded-lg ${compact ? 'p-1' : 'p-1.5'} transition-all text-center relative group
                ${isSelected
              ...
              `}
              style={{ width: `${100 / Math.min(forecast.length, 10)}%`, minWidth: compact ? '40px' : '46px' }}
            >
              {/* Day label */}
              <div className={`text-xs font-mono font-bold tracking-wider uppercase leading-none
              ...
              <div className="flex items-baseline justify-center gap-0.5">
                ...
                <span className="text-xs font-mono text-slate-500 leading-none">{day.airTempLow}°</span>
              </div>
              ...
              {!compact && (
                <>
                  ...
                  <div className="flex items-center justify-center gap-0.5 mt-0.5">
                    ...
                    <span className="text-xs font-mono text-sky-400/80 leading-none">{day.waterTemp}°</span>
                  </div>

                  {/* Wind */}
                  <div className="text-xs font-mono text-slate-600 mt-0.5 leading-none">
      ...
      {/* Selected day detail strip — hidden in compact mode */}
      {!compact && today && (
        <div className={`mt-2 rounded-lg px-3 py-2 flex items-center gap-4 text-xs font-mono
